...
export function CommandForm({
  ...
  readonly onPreview: (input: { readonly host: string; readonly command: string }) => void;
}) {
  ...
  return (
    <form className="action-form" onSubmit={(event) => {
      ...
      <button type="submit" disabled={disabled || busy || host.length === 0 || command.trim().length === 0}>生成确认预览</button>
    </form>
  );
}
