...
启动时可以通过 `--config <absolute-path>` 或 `SSH_MCP_CONFIG` 指定本文件。两者都未提供时，默认路径为进程当前工作目录下的 `ssh-mcp.yml`。如果默认文件不存在，服务会以仅当前用户可读写的方式生成一份模板，输出 `config.generated` 后正常退出；生成过程绝不覆盖已有文件。填写模板并再次启动后，服务才会加载配置并进入可操作状态。由于当前工作目录由 MCP 客户端决定，目录不明确时应继续显式指定绝对路径。
...
配置文件可以集中保存在用户私有目录中，例如 `~/.config/ssh-mcp/project-a.yml`，但启动时仍应传入展开后的绝对路径。实际配置不应提交到源码仓库，因为它可能暴露内部主机地址、用户名和本机路径；即使配置不含私钥内容，也应保持仅当前用户可读写。服务不合并全局配置与项目配置，也不根据当前项目自动选择配置；每个进程只加载明确指定的一份 YAML。
