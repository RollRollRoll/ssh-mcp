...
5. 控制台可用后写入一次性的 `console.ready`；客户端支持 URL elicitation 时，再通过标准协议请求客户端向用户呈现导航确认，用户接受后由客户端打开页面。URL 导航不可用或被拒绝不影响控制台与 `stdio`。
...
- 服务端只保存 token 摘要，不把 token 放入普通工具结果、普通日志、API 响应、HTML、JS bundle 或持久存储。完整访问 URL 只允许在一次性的 `console.ready` 诊断事件及客户端明确声明支持的 URL elicitation 请求中出现；后者由客户端先取得用户导航同意，不进入业务工具结果。
...
| MUST NOT 污染 MCP stdout | URL 只走结构化 stderr logger 或合法 URL elicitation 帧，且不进入工具结果；子进程协议测试持续验证 stdout |
...
- `console.ready` 结构化事件的 URL 字段名称及日志脱敏规则；完整访问 URL 只允许出现在该专用诊断事件与客户端明确支持的 URL elicitation 请求中。
