...
- 可观测：增加 `console.ready`、`console.request_rejected`、`console.stopped` 等稳定事件；不记录访问 token、Cookie、命令、Profile 参数或输出。
...
2. 生成实例 ID 与访问 token；`ConsoleServer` 在 `127.0.0.1:0` 建立 listener，但尚不对外宣布地址。
3. 连接 MCP `stdio` transport。
4. 两个入口均可用后，将控制台标记为 ready，并分别写入 `service.started` 与一次性的 `console.ready` 结构化诊断事件。
5. 任一步失败都按相反顺序关闭已建立资源；不得留下只启动 MCP 或只启动控制台的半活动实例。
...
若 HTTP listener 在运行期发生不可恢复错误，控制台触发整个 `ServerRuntime.shutdown()`；不允许 MCP 继续宣称完整可用而控制台永久失效。
...
- 服务端只保存 token 摘要，不把 token 放入普通日志、API 响应、HTML、JS bundle 或持久存储。完整访问 URL 只在 `console.ready` 诊断事件中输出一次，这是用户取得能力凭证的唯一刻意暴露点。
...
- `startServer()` 增加可注入的 `ConsoleServerFactory` test seam；生产默认创建真实 listener，既有测试可注入内存 fake。
...
- 子进程启动测试从 stderr `console.ready` 取得 URL，证明 stdout 仍只有 MCP JSON-RPC 帧；进程退出后旧地址不可访问。
...
| MUST NOT 自动打开浏览器 | 只写一次 `console.ready` URL，无系统 open 调用 |
...
| MUST NOT 污染 MCP stdout | URL 只走结构化 stderr logger；子进程协议测试持续验证 stdout |
...
- `console.ready` 结构化事件的 URL 字段名称及日志脱敏规则；必须保持“只允许该事件输出完整访问 URL”的专用白名单。
